...
function Activate_content() {
    ...
    console.log("page_title " +page_title.trim());
    switch(page_title.trim()){
        case "Repositories":
            $.ajax({
                type: "GET",
                url: "https://api.github.com/users/ruvenss/repos",
                dataType: "json",
                success: function(result) {
                    for( i in result ) {
                        $("#portfolio").append(
                            '<h3><a href="'+ result[i].html_url +'" target="_github">' + result[i].name +  '</a></h3>Created at '+ result[i].created_at +'<br><br>'
                        );
                        console.log("i: " + i);
                    }
                    console.log(result);
